...
def _presence(approval_callback=None) -> tuple:
    ...
    (-q) and kanban workers export HERMES_INTERACTIVE=1 but nobody answers prompts, and
    HERMES_EXEC_ASK has no human either — those flags are cleared so the unattended
    ...
    if _is_single_query_approval_context() or _is_kanban_approval_context():
