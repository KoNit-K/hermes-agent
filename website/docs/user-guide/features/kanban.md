...
| `kanban.auto_assign` | unset (off) | When `enabled: true` and `strategy: local-first-overflow`, unassigned ready tasks go to the first unsaturated `local_pool` profile, overflowing to `cloud_pool`. Explicit assignees and review-lane rows are never rewritten. |
